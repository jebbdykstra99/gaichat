(function () {
  ...
  let SITE_ID = 'gaichat';
  let site = null;
  let COLORS = ['#10231c', '#1aa37a', '#14815f', '#2d6b5a', '#1a3d32', '#3d6b5a'];
  let TRENDS = [];
  let PLACES = [];
  let TOPICS = [];
  ...
  function applySiteChrome() {
    if (!site) return;
    var title = site.name || "Generative AI";
  ...
  function syncProfile() {
    ...
    document.getElementById('profile-bio').textContent = currentUser.bio || "Tools, not oracles.";
    ...
    if (!pane) return;
    if (!mine.length) {
      pane.innerHTML = '<div class="empty-note" id="profile-posts-empty">No posts yet. Hit Post when you have a receipt, not a prophecy.</div>';
    } else {
  ...
  function stubSignIn(name, handle) {
    currentUser = {
      name: name || 'Guest',
      handle: (handle || 'guestgai').replace(/^@/, '').toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 15) || 'guestgai',
      bio: "Tools, not oracles.",
      live: false
    };
    saveJSON(LS_USER, currentUser);
    closeAuth();
  ...
  function wireEvents() {
    ...
    document.getElementById('cv-guest-login').addEventListener('click', function () { stubSignIn('Guest', 'guestgai'); });
    ...
    search.addEventListener('input', function () {
      ...
      function cards(list) {
        if (!list.length) return '<p class="empty-note">Nothing in the lab matched that.</p>';
        return list.map(function (c) {
  ...
  fetch(SITE_JSON_URL)
    ...
    .catch(function (e) {
      ...
      boot({ siteId: "gaichat", name: "Generative AI", tagline: "Tools, not oracles." });
    });
})();
